...
def test_behavior_taxonomy_prompt_requires_compact_runtime_policies():
    ...
    assert "action-observable feedback" in BEHAVIOR_TAXONOMY_USER
    assert "world-model feedback" in BEHAVIOR_TAXONOMY_USER
    assert "Do not force a fixed report_action_result" in BEHAVIOR_TAXONOMY_USER
