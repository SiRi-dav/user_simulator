...
def test_real_assistant_client_can_route_policy_and_response_to_different_host():
    calls = []
    ...
    client = RealAssistantClient(
        {
            "base_url": "http://query-trigger.test",
            "policy_base_url": "http://policy-response.test",
            "response_base_url": "http://policy-response.test",
            "common_sense_cases": "无需检索案例",
        },
        post_json=fake_post,
    )
    ...
    assert reply == "请问所有 Excel 都打不开吗？"
    assert [url for url, _, _ in calls] == [
        "http://query-trigger.test/query",
        "http://query-trigger.test/trigger",
        "http://policy-response.test/policy",
        "http://policy-response.test/response",
    ]
...
def test_real_assistant_client_skips_missing_policy_by_default():
    calls = []
    ...
    def fake_post(url, payload, timeout):
        ...
        if url.endswith("/trigger"):
            return ["预案", 0.2]
        if url.endswith("/policy"):
            from src.assistant.real_assistant_client import AssistantApiError

            raise AssistantApiError(404, url, '{"detail":"Not Found"}')
        if url.endswith("/response"):
            return ["请先确认 Excel 进程是否卡死。", 0.4]
        raise AssertionError(url)
    ...
    assert reply == "请先确认 Excel 进程是否卡死。"
    assert [url for url, _, _ in calls] == [
        "http://assistant.test/query",
        "http://assistant.test/trigger",
        "http://assistant.test/policy",
