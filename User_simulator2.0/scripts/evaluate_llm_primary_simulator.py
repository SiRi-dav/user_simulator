...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the LLM-primary, user-conditioned simulator evaluator without overwriting the old evaluator."
    )
    parser.add_argument("--config", help="Optional simulator config YAML. Reads llm and paths.dialogues/output_dir.")
    ...
    config = parse_simple_yaml(resolve_path(args.config)) if args.config else {}
    output_dir = resolve_path(args.output_dir or nested_get(config, ["paths", "output_dir"]) or "outputs")
